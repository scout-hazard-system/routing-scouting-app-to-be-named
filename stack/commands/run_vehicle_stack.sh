...
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-$ROOT_DIR/stack/docker-compose.server.yml}"
...
start_backend() {
  ...
  local build_with_fallback
  build_with_fallback() {
    (
      cd "$BACKEND_DIR" \
      && javac BackendServer.java \
      && printf 'Main-Class: BackendServer\n' > /tmp/vehicle_stack_manifest.mf \
      && jar cfm dist/backend-lite.jar /tmp/vehicle_stack_manifest.mf *.class
    )
  }
  if [[ -x "$BACKEND_DIR/build_executable.sh" ]]; then
    if ! "$BACKEND_DIR/build_executable.sh" >/dev/null; then
      printf "build_executable.sh failed; using direct javac/jar fallback.\n" >&2
      build_with_fallback
    fi
  else
    build_with_fallback
  ...
  nohup env \
    ...
    SELECTOR_SCRIPT_PATH="${SELECTOR_SCRIPT_PATH:-$ROOT_DIR/navigation/pipeline/channel_selector.py}" \
    BROADCASTIFY_CATALOG_SCRIPT_PATH="${BROADCASTIFY_CATALOG_SCRIPT_PATH:-$ROOT_DIR/navigation/pipeline/broadcastify_catalog_service.py}" \
    BROADCASTIFY_CHANNELS_FILE="${BROADCASTIFY_CHANNELS_FILE:-$ROOT_DIR/stack/config/broadcastify_channels.sample.json}" \
