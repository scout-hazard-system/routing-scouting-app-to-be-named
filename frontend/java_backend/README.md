...
## Files
- `BackendServer.java` - single-file Java HTTP server (no external deps).
...
PIPELINE_LOG_PATH=/tmp/pipeline_frontend_demo.log java BackendServer
```
...
```bash
JAVA_BACKEND_HOST=0.0.0.0 JAVA_BACKEND_PORT=8080 java BackendServer
```
...
```
Output:
- `dist/backend-lite.jar`

Run executable:
```bash
PIPELINE_LOG_PATH=/tmp/pipeline_live_doordash.log JAVA_BACKEND_PORT=8080 java -jar dist/backend-lite.jar
```
...
- `/api/platform/route/local` returns OSRM (OpenStreetMap) road route geometry (`route_points`), falling back to a direct origin->destination segment when OSRM is unreachable. Each route point now includes `zhs` unified geometry metadata (`z` tile index, `h` hex index, `s` spherical-cell index) for cross-system routing joins.
...
- Broadcastify selector helper is Ollama-managed by default (`BROADCASTIFY_SELECTOR_USE_OLLAMA_RERANK=true`, `BROADCASTIFY_SELECTOR_OLLAMA_MODEL=scout-rank`); set env overrides only when you intentionally want deterministic-only fallback behavior.
...
### Attribution
Map data is © OpenStreetMap contributors, licensed under ODbL.
- Dataset source: https://planet.openstreetmap.org
- License text: https://opendatacommons.org/licenses/odbl/1-0/
- Any UI displaying this data must show OpenStreetMap attribution and provide a data/license link.
- Source integrity policy: this stack reads upstream OSM sources in read-only mode and does not modify upstream OSM records.
- Derived tile/extract databases and outputs must continue to comply with ODbL share-alike terms.
...
- `scout-core1.0.3` - unified model for enforcement alert decision (`TASK: ALERT`), structured dispatch intel JSON extraction (`TASK: INTEL`), and spoken navigation guidance (`TASK: NAV`)
- `scout-vet1.0.4` - second-stage alert vet model (`TASK: VET`) gating emissions before client fanout
- `scout-rank` - channel selector reranking (set `BROADCASTIFY_SELECTOR_OLLAMA_MODEL=scout-rank`)

`GET /api/platform/llm/status` reports Ollama reachability and per-model availability. Env vars: `OLLAMA_TAGS_URL` (default `http://localhost:11434/api/tags`), `LLM_BASE_MODEL` (default `llama3.1`). The Python client layer (`llm_set_client.py`) falls back to inline `llama3.1` prompts when scout models are not built.
